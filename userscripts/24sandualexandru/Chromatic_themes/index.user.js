    ...
    return EMOJIS_GRADIENTS[code];
         ...
         if(node.classList.contains("instance-discovery"))
            ...
            if(fdSparkle)
             ...
             const pseudoElementStyles = window.getComputedStyle(node, '::after');
              if(node.querySelector(".fake-sparkle"))
                 {
                   node.removeChild(node.querySelector(".fake-sparkle"));
                 }
                ...
                fakeDiv.style.position="absolute";
                 fakeDiv.style.position = 'absolute';
                 fakeDiv.style.top = pseudoElementStyles.getPropertyValue('top');
                 fakeDiv.style.left = pseudoElementStyles.getPropertyValue('left');
                 fakeDiv.style.width = pseudoElementStyles.getPropertyValue('width');
                 fakeDiv.style.height = pseudoElementStyles.getPropertyValue('height');
                 fakeDiv.style.transform = "translateX(-25%)"
                 fakeDiv.classList.add("fake-sparkle");



                 //fakeDiv.style.transform="translateX(-50%)";
                 fakeDiv.height="30px";
                 var img=document.createElement("img");
                 img.src=`data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" width="20px" height="20px"><path fill="%23`+ emojiAverageColor.substring(1) +`" d="M49.306 26.548l-11.24-3.613-3.613-11.241C34.319 11.28 33.935 11 33.5 11s-.819.28-.952.694l-3.613 11.241-11.24 3.613C17.28 26.681 17 27.065 17 27.5s.28.819.694.952l11.24 3.613 3.613 11.241C32.681 43.72 33.065 44 33.5 44s.819-.28.952-.694l3.613-11.241 11.24-3.613C49.72 28.319 50 27.935 50 27.5S49.72 26.681 49.306 26.548zM1.684 13.949l7.776 2.592 2.592 7.776C12.188 24.725 12.569 25 13 25s.813-.275.948-.684l2.592-7.776 7.776-2.592C24.725 13.813 25 13.431 25 13s-.275-.813-.684-.949L16.54 9.459l-2.592-7.776C13.813 1.275 13.431 1 13 1s-.813.275-.948.684L9.46 9.459l-7.776 2.592C1.275 12.188 1 12.569 1 13S1.275 13.813 1.684 13.949zM17.316 39.05l-5.526-1.842-1.842-5.524C9.813 31.276 9.431 31 9 31s-.813.275-.948.684L6.21 37.208.685 39.05c-.408.136-.684.518-.684.949s.275.813.684.949l5.526 1.842 1.841 5.524C8.188 48.721 8.569 48.997 9 48.997s.813-.275.948-.684l1.842-5.524 5.526-1.842C17.725 40.811 18 40.429 18 39.999S17.725 39.186 17.316 39.05z"/></svg>`;
                 fakeDiv.appendChild(img);
                 node.appendChild(fakeDiv);




             node.style.setProperty("--second-color",emojiAverageColor);
             node.style.setProperty("--main-color","color-mix( in srgb,"+emojiAverageColor+",#fff 60%)");
             node.style.setProperty("--after-content","\"First Discovery\"");
               }else
                 {
                   node.style.setProperty("--after-content","\"\"");
                 }
...
''
    ...
    node.style.transition = "border-image-source 300ms ease, background-image 300ms ease";
      ...
      .instance-discovery::after {
       --linear-direction:to right;
          content: var(--after-content) !important;
          display: inline-block;
        -webkit-animation      : background-pan 10s linear infinite !important;
        animation              : background-pan 10s linear infinite !important;
        ...
        background-image: var(--image-path), linear-gradient( var(--bg-angle),  var(--main-color), var(--second-color) ) !important;
        ...
        `;
         ...
         document.getElementsByTagName('head')[0].appendChild(style);
          ...
          function set_up_color_settings_button()
          {
            ...
            if(!content.contains(selectionDiv))
